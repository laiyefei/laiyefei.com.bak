...
(function ($) {
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function () {
            ...
            dateeditorHTML += "        <div class='l-box-dateeditor-yearselector'><div class='l-box-dateeditor-yearselector-arrow l-box-dateeditor-yearselector-arrow-left'>&lt;&lt;</div><ul><li></li><li></li><li></li><li></li><li></li><li></li><li></li><li></li><li></li><li></li><li></li><li></li></ul><div class='l-box-dateeditor-yearselector-arrow l-box-dateeditor-yearselector-arrow-right'>&gt;&gt;</div></div>";
            ...
            g.buttons.btnYear.click(function () {
                ...
                if (!g.body.yearselector.is(":visible")) {
                    $("li", g.body.yearselector.find("ul")).each(function (i, item) {
            ...
            $("li", g.body.yearselector.find("ul")).click(function () {
                g.currentDate.year = parseInt($(this).html());
                g.body.yearselector.slideToggle();
                g.bulidContent();
            });
            $(".l-box-dateeditor-yearselector-arrow", g.body.yearselector).click(function () {
                //向右
                var arrowType = ''
                if ($(this).hasClass("l-box-dateeditor-yearselector-arrow-right")) {
                    arrowType = 'right';
                } else {
                    //向左
                    arrowType = 'left';
                ...
                $("li", g.body.yearselector.find("ul")).each(function (i, item) {
                    var liYear = 0;
                    if (arrowType == 'left') {
                        liYear = Number($(this).text()) - 12;
                    } else {
                        liYear = Number($(this).text()) + 12;
                    }
                    if (liYear == g.currentDate.year) {
                        $(this).addClass("l-selected");
                    } else {
                        $(this).removeClass("l-selected");
                    }
        ...
        _render: function () {
            ...
            if (p.buttons) {
                $(p.buttons).each(function (i, item) {
                    ...
                    //#region add by lqh
                    //判断是否需要验证提交状态
                    var verifySubmit = item.verifySubmit || false;
                    if (verifySubmit) {
                        btn.addClass("l-dialog-btn-submit");
                    }

                    item.onclick && btn.click(function () {
                        //如果需要验证提交状态，且当前状态不在提交时
                        if (item.verifySubmit) {
                            if (btn.hasClass("l-dialog-btn-submit")) {
                                btn.removeClass("l-dialog-btn-submit").addClass("l-dialog-btn-submit-loading");
                                $(".l-dialog-btn-inner", btn).html(item.submitText || item.text);
                                //提交后回调
                                var submited = function () {
                                    btn.removeClass("l-dialog-btn-submit-loading").addClass("l-dialog-btn-submit");
                                    $(".l-dialog-btn-inner", btn).html(item.text);
                                }
                                item.onclick(item, g, submited);
                            } else {
                                return;
                            }
                        } else {
...
(function ($) {
    ...
    $.ligerDefaults.Grid = {
        ...
        pageParmName: 'pageNum',               //页索引参数名，(提交给服务器)
        ...
        updateRowCallback: null,  //修改行数据之后的回调函数 hl于2017-06-05 添加

        //显示详情修改
        isDetailBtn: false, 		//明细列是否要默认的按钮  hl于2017-06-13 添加
        detailRowClick: true,    		//行是否支持行明细单击事件  hl于2017-06-13 添加
        isDetailOneShow: true			//明细点击是否显示一个（只适用于点击行的时候）  hl于2017-06-13 添加
    ...
    $.ligerDefaults.Grid_columns = {
        ...
        editor: null,
        render: null,
        isDblClickEven: true, 	//双击之后是否处方触发行事件 hl于17-06-30添加
        textField: null, //真正显示的字段名,如果设置了，在编辑状态时,会调用创建编辑器的setText和getText方法
        required: false //该列是否必填，如果必填则修改该列的背景色
    };
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        loadServerData: function (param, clause, sourceType) {
            ...
            var ajaxOptions = {
                ...
                beforeSend: function (xhr) {
                    if (typeof (p.xAuthToken) == "function") {
                        p.xAuthToken(xhr);
                    }
                ...
                success: function (data) {
                    //adder : leaffly
                    if ($.ligeruiBuff) {
                        var authData = $.ligeruiBuff.authentication(data);
                        if (!authData["ok"]) {
                            return;
                        }

                        if ($.ligeruiBuff.format != undefined && "function" == typeof $.ligeruiBuff.format) {
                            data = $.ligeruiBuff.format(data);
                        } else {
                            console.error("sorry, this ligerui you use is need dependent ligerui.all.buff.js and format function is must need.");
                        }
                    } else {
                        console.error("sorry, this ligerui you use is need dependent ligerui.all.buff.js .");
                    }
        ...
        //删除所有行 hl于2017-07-03 添加
        deleteAllRow: function () {
            var g = this, p = this.options;
            var rowdata;
            for (var i = 0; i < g.recordNumber; i++) {
                g._deleteData(i);
            }
            g.reRender();
            g.isDataChanged = true;
        ...
        //增加下一行 hl 于2017-06-05添加
        addNextRow: function (index, rowdata) {
            var g = this, p = this.options;
            ...
            var listdata = g[p.root];
            if (index == -1 || index == listdata.length - 2) return;
            g.move(rowdata, listdata[index + 1], false);
        },
        ...
        updateRow: function (rowDom, newRowData) {
            ...
            if (typeof p.updateRowCallback == "function") {
                p.updateRowCallback.ligerDefer(this, 30);
            }
            return rowdata;
        },
        ...
        _initColumns: function () {
            ...
            //明细列
            if (g.enabledDetail() && p.isDetailBtn) { // 隐藏详情按钮列 hl于2016-06-13添加
                // if (g.enabledDetail() ) { // 原本的代码为左侧，
        ...
        _fillGridBody: function (data, frozen, sourceType) {
            ...
            //全选/全不选 复选框选中事件 hl于170821 添加
            var hrow = g.gridheader.find(".l-grid-hd-row");
            var isAllSelect = g.selected.length == g.rows.length;
            if (isAllSelect) { //全选
                if (!$(hrow).hasClass("l-checked")) {
        ...
        _getHtmlFromData: function (data, frozen) {
            ...
            for (var i = 0, l = data.length; i < l; i++) {
                ...
                $(g.columns).each(function (columnindex, column) {
                    ...
                    if (column.required) {
                        gridhtmlarr.push(' class="l-grid-row-cell col-required');
                    } else {
        ...
        _getCellHtml: function (rowdata, column) {
            ...
            var title = column["name"] ? rowdata[column["name"]] : null;
            ...
            if (title) {
                if (column.type == "date") {
                    title = common.dateTimeStamp2dateTime(title, column.format);
                }
                htmlarr.push(' title="' + title + '"');
            }
        ...
        _buildPager: function () {
            ...
            //update by lqh  当表格界面小于560px时，分页提醒内容只显示总条数
            var gWidth = $(g.grid[0]).width();
            if (gWidth < 560) {
                stat = "总 {total} 条";
                stat = stat.replace(/{total}/, p.total);
            }
        ...
        _onDblClick: function (e) {
            ...
            //添加isDblClickEven 判断 hl于17-06-30添加
            var index = 0;
            if (src.cell) {
                index = src.cell.cellIndex - 1;
                if (index < 0) return;
            }
            if (src.cell && p.columns[index].isDblClickEven != false) {
                if (src.row) {
        ...
        _onClick: function (e) {
            ...
            if (src.checkboxall) //复选框全选
            {
                ...
                for (var rowid in g.records) {
                    ...
                    if (uncheck) {
                        ...
                        g.trigger('checkRow', [false, g.records[rowid], g.records[rowid]["__id"]]);
                    } else {
                        g.select(g.records[rowid]);
                        g.trigger('checkRow', [true, g.records[rowid], g.records[rowid]["__id"]]);
                    }
            ...
            //明细
                ...
                //之前上面的代码为   else if ((src.detailbtn && p.detail)）{   hl于2017.06.13修改
                ...
                if ($(src.detailbtn).hasClass("l-open") || $(row).hasClass("l-open")) {
                    // 之前上面的代码为  if($(src.detailbtn).hasClass("l-open")){  hl于2017.06.13修改

                    if (p.detail.onCollapse)
                        p.detail.onCollapse(item, $(".l-grid-detailpanel-inner:first", nextrow)[0]);
                    row.next("tr.l-grid-detailpanel").hide();

                    //  $(src.detailbtn).removeClass("l-open"); // 原本的代码
                    //hl于2017.06.13修改
                    if (p.detailRowClick) {
                        $(row).removeClass("l-open");
                    } else {
                    ...
                    if (nextrow.length > 0) {
                        ...
                        // $(src.detailbtn).addClass("l-open"); //之前的代码
                        //hl于2017.06.13修改
                        if (p.detailRowClick) {
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                o[p.textFieldName] = o[p.textFieldName].replace(/>/g, '&gt;').replace(/</g, '&lt;').replace(/"/g, "&quot;").replace(/'/g, "&#39;");


                var title = "title='" + o[p.textFieldName] + "'";


                if (p.render) {
                    treehtmlarr.push('<span ' + title + '>' + p.render(o, o[p.textFieldName]) + '</span>');
                } else {
                    treehtmlarr.push('<span ' + title + '>' + o[p.textFieldName] + '</span>');
                }
                treehtmlarr.push('</div>');
                if (g.hasChildren(o)) {
                    var isLastNew = [];
